...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%生成麦克风位置朝向信息
    Microphone_Cita=fix(360*(rand(Node_All,1)));  %%朝向 [0  360]    
...
%     Xnode_All=Microphone_Center_Location; % 节点中心坐标
    ...
    Xanchor=Node_Location(1:Anchor_Number*2,:);  % 锚节点坐标
    Xnode=Node_Location(Anchor_Number*2+1:end,:); % 普通节点坐标
...
%   rmse_angle_tmp(runs)  = sqrt(sum(angle_temp-Microphone_Cita).^2 / Node_Number);   %
  %%%%%%% 估计位置
  xxest_msp= MSP_EDSNL(Anchor_Number*2,Node_Number*2,Scan_Time,Xanchor,X_rank,cita,p_band,Xnode) ;
 
%   Calculate slope
angle = zero(Node_Number);
 for i =1:Node_All
